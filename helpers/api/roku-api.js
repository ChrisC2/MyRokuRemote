...
const getListener = (client) => () => {
  const params = [...getQuery(), () => setTimeout(() => client.close(), 3500)];
  client.send(...params);
};
...
export const getDeviceApps = async (deviceUrl) => {
  try {
    ...
    const {
      apps: {app: apps},
    } = await convert.xml2js(xml, {compact: true});
    const deviceApps =
      apps.length &&
      (await Promise.all(
        apps.map(async ({_attributes: {id}, _text: appName}) => {
...
export const onLoad = async () => {
  try {
    let rokuDevices = await discoverDevices();

    if (rokuDevices.length) {
      rokuDevices = await Promise.all(
        rokuDevices.map(async ({ip, deviceName, id}) => {
          const apps = await getDeviceApps(ip);
          return {
            ip,
            apps,
            deviceName,
